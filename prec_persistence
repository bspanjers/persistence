...
test = QAR_climate(sCity='DE BILT', fTau=.95, use_statsmodels=True, include_nao=True,)
test.prepare_data()
...
y_prec_old = (test.temp.Temp >= mm_threshold) * 1
data_old = pd.DataFrame(y_prec_old, columns=['Temp'])
data_old['nao_index_cdas'] = test.temp.nao_index_cdas
...
#mB = simulate_paths(df_sorted_new, seasonal_results_new, y_prec_new_season, iS=10000)
